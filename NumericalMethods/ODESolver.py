...
'''
def ForwardEuler_SIR(S0, I0, R0, beta, v, dt, T):
    n = floor(T / dt)
    print(n)
    S = np.zeros(n + 1)
    I = np.zeros(n + 1)
    ...
    R[0] = R0
    for k in range(n):
        S[k + 1] = S[k] - beta * S[k] * I[k] * dt
        I[k + 1] = I[k] + (beta * S[k] * I[k] - v * I[k]) * dt
        R[k + 1] = R[k] + v * I[k] * dt
        t[k + 1] = t[k] + dt
    return [S, I, R, t]
'''
...
class BackwardEuler(ODESolver):
    ...
    def __init__(self, f):
        ...
        try:
            ...
            raise ImportError('''Could not import module "Newton". Place Newton.py in this directory(%s)''' % (os.path.dirname(os.path.abspath(__file__))))
